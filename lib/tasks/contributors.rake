  ...
  task :recent_activity, [:collection_id] => :environment do |t, args|
    ...
    unless new_transcribers.empty?
      ...
      new_transcribers.each do |t|
        puts "<a href='#{Rails.application.routes.url_helpers.url_for({ controller: 'user', action: 'profile',
                                                                        user_id: t.id })}'>#{t.display_name}</a>  #{t.email} <br>"
      end
    else
    ...
    unless recent_transcriptions.empty?
      ...
      recent_transcriptions.each do |t|
        user_url = Rails.application.routes.url_helpers.url_for({ controller: 'user', action: 'profile',
                                                                  user_id: t.user.id })
        page_url = Rails.application.routes.url_helpers.url_for({ controller: 'display', action: 'display_page',
                                                                  page_id: t.page.id })
        work_url = Rails.application.routes.url_helpers.url_for({ controller: 'display', action: 'read_work',
                                                                  work_id: t.work.id })
