  ...
  task :weekly_trial_cohort => :environment do
    ...
    week_cohorts.each do |start_day|
      ...
      target_actions.each do |action|
        if previous_visits
          visits = Ahoy::Event.where(time: start_day..end_day, name: action,
                                     visit_id: previous_visits).pluck(:visit_id).uniq
      ...
      ids_with_activity = users_with_pages.select { |u|
        u.owner_works.detect { |w|
      ...
      ids_with_multiple_contributors = users_with_activity.select { |u|
        u.owned_collections.detect { |c|
      ...
      users_with_pages_transcribed.each do |user|
        first_edit_dates = user.collections.map { |c|
          c.deeds.where(deed_type: DeedType.collection_edits).minimum(:created_at)
        }
