module ContributorHelper
  def new_contributors(collection_id, start_date, end_date)
    ...
    @recent_translations = @collection_deeds.where(deed_type: [DeedType::PAGE_TRANSLATED,
                                                               DeedType::PAGE_TRANSLATION_EDIT])
  ...
  def activity(collection, hours)
    ...
    new_works = Work.includes(:collection).where(collection_id: collection.id).where("created_on > ?",
