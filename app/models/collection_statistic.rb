module CollectionStatistic
  ...
  def mention_count(last_days = nil)
    Collection.count_by_sql("SELECT COUNT(*) FROM page_article_links pal INNER JOIN articles a ON pal.article_id = a.id WHERE a.collection_id = #{self.id}  #{last_days_clause(
      ...
      stats =
        {
          ...
          :pages => self.works.joins(:pages).where(timeframe(start_date, end_date,
                                                             'pages.created_on')).count,
          :subjects => self.articles.where(timeframe(start_date, end_date, 'created_on')).count,
          :mentions => self.articles.joins(:page_article_links).where(timeframe(start_date, end_date,
          ...
          :pages_transcribed => self.pages.where(status: Page::COMPLETED_STATUSES).where(timeframe(start_date,
                                                                                                   end_date, 'pages.edit_started_at')).count,
          :works_transcribed => self.works.joins(:work_statistic).where(work_statistics: { complete: 100 }).where(timeframe(
                                                                                                                    start_date, end_date, 'work_statistics.updated_at'
                                                                                                                  )).count,
          :pages_incomplete => self.pages.where(status: Page::NEEDS_WORK_STATUSES).where(timeframe(start_date,
                                                                                                   end_date, 'pages.edit_started_at')).count,
          :pages_needing_review => self.pages.where(status: :needs_review).where(timeframe(start_date, end_date,
  ...
  def calculate_complete
    ...
    unless page_count == 0
      ...
      pct = (self.works.includes(:work_statistic).map { |w|
