module DocumentSetStatistic
  ...
  def contributor_count(last_days = nil)
    User.joins(:deeds).where(deeds: { work_id: works.ids }).where("#{last_days_clause(last_days,
                                                                                      'deeds.created_at')}").distinct.count
  end
    ...
    stats =
      {
        ...
        :pages_transcribed => self.pages.where(status: Page::COMPLETED_STATUSES).where(timeframe(start_date, end_date,
