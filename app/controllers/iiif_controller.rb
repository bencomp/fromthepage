...
class IiifController < ApplicationController
  ...
  def contributions
    ...
    begin
      ...
      render :status => 400,
             :plain => "Could not parse #{raw_terminus_a_quo} as a date. Try a format like #{DateTime.now.iso8601}"
      return
    end
    begin
      ...
      render :status => 400,
             :plain => "Could not parse #{raw_terminus_ad_quem} as a date. Try a format like #{DateTime.now.iso8601}"
      return
    end
  ...
  def collection_for_domain(domain, terminus_a_quo = nil, terminus_ad_quem = nil)
    if domain.include? '.'
      ...
      if terminus_a_quo && terminus_ad_quem
        ...
        works = Work.joins(:work_statistic, :sc_manifest).where(
          "sc_manifests.at_id LIKE ? AND work_statistics.last_edit_at >= ?", "%#{domain}%", terminus_a_quo
        ).distinct.includes(:work_statistic)
      else
        works = Work.joins(:sc_manifest).where("sc_manifests.at_id LIKE ?", "%#{domain}%").includes(:work_statistic,
                                                                                                    :sc_manifest)
      end
      ...
      if user && user.owner
        if terminus_a_quo && terminus_ad_quem
          ...
          works = Work.joins(:collection).where("collections.owner_user_id = ?", user.id).includes(:work_statistic,
                                                                                                   :sc_manifest)
        end
      else
        works = []
  ...
  def manifest
    ...
    seed = {
      '@id' => url_for({ :controller => 'iiif', :id => work_id, :action => 'layer', :type => 'transcription',
                         :only_path => false }),
    ...
    if work.supports_translation?
      seed = {
        '@id' => url_for({ :controller => 'iiif', :id => work_id, :action => 'layer', :type => 'translation',
                           :only_path => false }),
    ...
    if true # any notes
      seed = {
        '@id' => url_for({ :controller => 'iiif', :id => work_id, :action => 'layer', :type => 'notes',
                           :only_path => false }),
        'label' => "notes layer"
      }
      layer = IIIF::Presentation::Layer.new(seed)
  ...
  def list
    ...
    annotation_list['@id'] =
      url_for({ :controller => 'iiif', :action => 'list', :page_id => @page.id, :annotation_type => type,
                :only_path => false })
  ...
  def layer
    ...
    if params[:type] == "transcription"
      seed = {
        '@id' => url_for({ :controller => 'iiif', :id => work_id, :action => 'layer', :type => params[:type],
                           :only_path => false }),
        'label' => 'Transcription'
      }
      layer = IIIF::Presentation::Layer.new(seed)
  ...
  def structured_data_endpoint
    ...
    if @page && !@page.notes.blank?
      response['notes'] =
        url_for({ :controller => 'iiif', :action => 'list', :page_id => @page.id, :annotation_type => 'notes',
                  :only_path => false })
    end
    if @page
  ...
  def annotation
    ...
    annotation['@id'] =
      url_for({ :controller => 'iiif', :action => 'annotation', :page_id => @page.id, :annotation_type => type,
                :only_path => false })
  ...
  def notes
    ...
    annotation_list = IIIF::Presentation::AnnotationList.new
    annotation_list['@id'] =
      url_for({ :controller => 'iiif', :action => 'notes', :page_id => @page.id, :only_path => false })
    @page.notes.each_with_index do |note, i|
      note = iiif_page_note(@page, i + 1)
      note['@id'] =
        url_for({ :controller => 'iiif', :action => 'note', :page_id => @page.id, :note_id => i + 1,
                  :only_path => false })
  ...
  def note
    ...
    note['@id'] =
      url_for({ :controller => 'iiif', :action => 'note', :page_id => @page.id, :note_id => noteid,
                :only_path => false })
  ...
  def iiif_page_note(page, noteid)
    ...
    note.resource = IIIF::Presentation::Resource.new({ '@id' => "note_#{noteid}_for_#{@page.id}",
  ...
  def iiif_sequence_from_work_id(work_id)
    ...
    sequence['@id'] =
      url_for({ :controller => 'iiif', :action => 'sequence', :work_id => work_id, :sequence_name => 'default',
                :only_path => false })
  ...
  def iiif_collection_id_from_collection(collection)
    if collection.is_a? DocumentSet
      ...
      url_for({ :controller => 'iiif', :action => 'collection', :collection_id => collection.slug,
                :only_path => false })
    end
  end
  ...
  def canvas_id_from_page(page)
    if page.sc_canvas
      ...
      url_for({ :controller => 'iiif', :action => 'canvas', :page_id => page.id, :work_id => page.work.id,
                :only_path => false })
    end
  end
  ...
  def add_annotations_to_canvas(canvas, page)
    unless page.source_text.blank?
      annotation_list = IIIF::Presentation::AnnotationList.new
      annotation_list['@id'] =
        url_for({ :controller => 'iiif', :action => 'list', :page_id => page.id, :annotation_type => "transcription",
                  :only_path => false })
    ...
    unless page.source_translation.blank?
      annotation_list = IIIF::Presentation::AnnotationList.new
      annotation_list['@id'] =
        url_for({ :controller => 'iiif', :action => 'list', :page_id => page.id, :annotation_type => "translation",
                  :only_path => false })
  ...
  def add_related_to_canvas(canvas, page)
    ...
    canvas.related << { "label" => "Read this page", "format" => "text/html",
                        "@id" => url_for(:controller => :display, :action => :display_page, :page_id => page.id) }
    canvas.related << { "label" => "Transcribe this page", "format" => "text/html",
                        "@id" => url_for(:controller => :transcribe, :action => :display_page, :page_id => page.id) }
    canvas.related << { "label" => "Translate this page", "format" => "text/html",
                        "@id" => url_for(:controller => :transcribe, :action => :translate, :page_id => page.id) } if page.work.supports_translation?
  end
  ...
  def add_seeAlso_to_canvas(canvas, page)
    ...
    canvas.seeAlso <<
      { "label" => "HTML Transcription",
        ...
        "@id" => collection_annotation_page_transcription_html_url(page.work.owner, page.work.collection, page.work,
                                                                   page) }
    canvas.seeAlso <<
      { "label" => "Searchable Plaintext",
        "format" => "text/plain",
    ...
    if page.work.supports_translation? && !page.source_translation.blank?
      canvas.seeAlso <<
        { "label" => "HTML Translation",
          ...
          "@id" => collection_annotation_page_translation_html_url(page.work.owner, page.work.collection, page.work,
                                                                   page) }
      canvas.seeAlso <<
        { "label" => "Verbatim Translation Plaintext",
          "format" => "text/plain",
  ...
  def status_service_for_manifest(work)
    ...
    service["@id"] =
      url_for({ :controller => 'iiif', :action => 'manifest_status', :work_id => work.id, :only_path => false })
  ...
  def status_service_for_page(page)
    ...
    service["@context"] = "http://www.fromthepage.org/jsonld/1/context.json"
    service["@id"] =
      url_for({ :controller => 'iiif', :action => 'canvas_status', :work_id => page.work.id, :page_id => page.id,
                :only_path => false })
    service["pageStatus"] = []
