...
RSpec.describe SubjectExporter do
  describe 'sections export' do
    ...
    let(:links_a)    {
      FactoryBot.build_list(:page_article_link, 2, display_text: 'display_text_a', article: article_a)
    }
    let(:links_b) {
      FactoryBot.build_list(:page_article_link, 2, display_text: 'display_text_b', article: article_b)
    }
    ...
    let(:collection) {
      FactoryBot.create(:collection, works: [work_1, work_2], owner: user, articles: [article_a, article_b])
    }
